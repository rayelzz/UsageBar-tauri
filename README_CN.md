...
- 顶部 / 底部贴边时，百分比显示在圆环右侧
- 鼠标不在条上时默认点穿，不挡后面窗口
- 条末端外侧有一个悬空的设置齿轮（原来的条身形状不动）。悬停出现，点击打开和 **UB** 同一套深色菜单；点空白处关闭
...
条上的设置齿轮、菜单栏 / 托盘 **UB**，或右键圆环条：立即刷新、自动刷新、锁定位置、不阻挡下方点击、贴左 / 右 / 上 / 下、显示样式、显示值（已使用额度 / 剩余额度）、语言（默认 English，可切 中文；厂商和模型名保持英文）、**提供商…**（1–10 个，上下排序，条长随数量变化）、当前版本 / **检测更新** / 最新版本（点击安装）、自动检测更新（默认关闭）、登录时打开、退出。检测到新版本时会按语言弹出 Release 里的更新说明。点菜单外的空白处即可关闭。

贴边、位置、提供商个数和顺序、显示样式、显示值、语言、刷新、锁定、点穿、更新等全部设置都写在本机 `~/.usagebar/prefs.json`，重启或更新应用后会按上次配置恢复。
