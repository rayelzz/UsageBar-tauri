...
- **自动检测更新默认关闭**。在 **工具…** 或齿轮菜单里打开后，有新版本会弹出气泡。工具窗口会显示当前版本，也可手动检测；点最新版本号跳转下载。点过气泡跳转后，该版本不再提醒，等到下一个版本才会再提示
...
条上的设置齿轮、菜单栏 / 托盘 **UB**，或右键圆环条：立即刷新、自动刷新、锁定位置、不阻挡下方点击、贴左 / 右 / 上 / 下、显示样式、语言（默认 English，可切 中文；厂商和模型名保持英文）、**工具…**（1–10 个，上下排序，条长随数量变化；同时显示当前版本、手动检测更新、自动检测更新开关）、自动检测更新（默认关闭）、登录时打开、退出。点菜单外的空白处即可关闭。
