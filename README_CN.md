...
- 可拖动，自动贴左 / 右 / 上 / 下
- 两种显示样式：**圆环用量**（完整圆环 + 百分比 + 黑底托）或 **透明图标**（同样的圆环和百分比，无底托）。齿轮菜单里还有**不透明度**和**模糊**；macOS 上毛玻璃跟条和圆角卡片走。不透明度低于 60% 时字和图标变深
- 显示值：**已使用额度**（默认）或 **剩余额度**。圆环、百分比和详情都按这个值显示；颜色仍按剩余额度（剩余 ≤ 20% 红，≤ 40% 黄，其余绿）
- 顶部 / 底部贴边时，百分比显示在圆环右侧
- 鼠标不在条上时默认点穿，不挡后面窗口
- 条末端外侧有一个悬空的设置齿轮（原来的条身形状不动）。悬停出现，点击打开和 **UB** 同一套深色菜单；点 **×** 或空白处关闭
...
- 状态栏 / 托盘是文字 **UB**（macOS）。点齿轮、右键圆环条或点 **UB** 打开同一套菜单
- 默认 60 秒刷新，可改
...
条上的设置齿轮、菜单栏 / 托盘 **UB**，或右键圆环条：立即刷新、自动刷新、锁定位置、不阻挡下方点击、贴左 / 右 / 上 / 下、显示样式、显示值（已使用额度 / 剩余额度）、面板不透明度、模糊半径、语言（默认 English，可切 中文；厂商和模型名保持英文）、**提供商…**（同一弹出层里勾选 1–10 个并上下排序，条长随数量变化）、当前版本 / **检测更新** / 最新版本（点击安装）、自动检测更新（默认关闭）、登录时打开、退出。检测到新版本时会按语言弹出 Release 里的更新说明。点右上角 **×** 或菜单外空白处即可关闭。

贴边、位置、提供商个数和顺序、显示样式、显示值、不透明度、模糊、语言、刷新、锁定、点穿、更新等全部设置都写在本机 `~/.usagebar/prefs.json`，重启或更新应用后会按上次配置恢复。
