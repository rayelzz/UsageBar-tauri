...
function t() {
  return isZh()
    ? {
        ...
        toolsBack: "返回",
        barOpacity: "面板不透明度",
        barOpacityHint: "越低越透明，字会变深",
        barBlur: "模糊半径",
        barBlurHint: "越高越糊，透明时更明显",
        lookReset: "还原默认",
    ...
    : {
        ...
        barOpacityHint: "Lower = more transparent",
        barBlur: "Blur radius",
        barBlurHint: "Higher = blurrier",
        lookReset: "Reset",
...
const LogicalSize = () => api().window.LogicalSize;
const LogicalPosition = () => api().window.LogicalPosition;
...
function radii(w, h, along) {
  const base = Math.min(w, h);
  let fillet = base * 0.35;
  let corner = base * 0.42;
  const limit = along / 2;
  const total = fillet + corner;
...
function logicalMonitor(m) {
  ...
  return {
    ...
    w: m.size.width / s,
    h: m.size.height / s,
    wx: m.workArea.position.x / s,
    wy: m.workArea.position.y / s,
    ww: m.workArea.size.width / s,
    wh: m.workArea.size.height / s,
    scale: s,
    raw: m,
  };
}
...
function nearest(frame, screen) {
  const right = Math.abs(frame.x + frame.w - (screen.x + screen.w));
  const left = Math.abs(frame.x - screen.x);
  const top = Math.abs(frame.y - screen.wy);
  const bottom = Math.abs(frame.y + frame.h - (screen.wy + screen.wh));
  const pairs = [
    ["right", right],
    ["left", left],
    ["top", top],
  ...
  return pairs[0][1] <= SNAP ? pairs[0][0] : "floating";
}

function gluedFrame(edge, size, along, screen) {
  if (edge === "right") {
    const y = clamp(along, screen.wy, screen.wy + screen.wh - size.h);
    return { x: screen.x + screen.w - size.w + 1, y, w: size.w, h: size.h };
  }
  if (edge === "left") {
    const y = clamp(along, screen.wy, screen.wy + screen.wh - size.h);
    return { x: screen.x - 1, y, w: size.w, h: size.h };
  }
  if (edge === "top") {
    const x = clamp(along, screen.wx, screen.wx + screen.ww - size.w);
    return { x, y: screen.wy, w: size.w, h: size.h };
  }
  if (edge === "bottom") {
    const x = clamp(along, screen.wx, screen.wx + screen.ww - size.w);
    return { x, y: screen.wy + screen.wh - size.h, w: size.w, h: size.h };
  }
  const x = clamp(along, screen.wx, screen.wx + screen.ww - size.w);
  return { x, y: screen.wy + 80, w: size.w, h: size.h };
}

async function monitors() {
  const list = await api().window.availableMonitors();
  return list.map(logicalMonitor);
}

async function monitorAt(point, list) {
  return (
    list.find(
      (s) =>
        point.x >= s.x &&
...
async function resolveScreen(list, point) {
  if (prefs.screenName) {
    const named = list.find((s) => s.name === prefs.screenName);
    if (named) return named;
  }
  if (point) return monitorAt(point, list);
  try {
    const primary = await api().window.primaryMonitor();
    if (primary) return logicalMonitor(primary);
  } catch {
    /* fall through */
  }
  return list[0];
...
function inSystemBarBand(point, screen) {
  const topBand = screen.wy - screen.y;
  if (
    topBand > 8 &&
    point.y >= screen.y - 2 &&
    point.y <= screen.wy + 4 &&
  ...
  const bottomBand = screen.y + screen.h - (screen.wy + screen.wh);
  if (
    bottomBand > 8 &&
    point.y >= screen.wy + screen.wh - 4 &&
    point.y <= screen.y + screen.h + 2 &&
    point.x >= screen.x &&
    point.x <= screen.x + screen.w
  ) {
    return true;
...
async function placeWindows() {
  ...
  try {
    await bar.setMinSize(new (LogicalSize())(1, 1));
  } catch {
    /* older runtime */
  }
...
function paintHover() {
  document.querySelectorAll("[data-id]").forEach((el) => {
    el.classList.toggle("hovered", el.dataset.id === hovered);
  });
}
...
function restartTimer() {
  ...
  if (prefs.refreshInterval > 0) {
    refreshTimer = setInterval(refresh, prefs.refreshInterval * 1000);
  }
}
...
async function snapTo(edge) {
  ...
  const size = barSize(edge);
  const scale = await getCurrentWindow().scaleFactor();
  const frame = { x: pos.x / scale, y: pos.y / scale, w: size.w, h: size.h };
  const list = await monitors();
  const screen = await monitorAt({ x: frame.x + frame.w / 2, y: frame.y + frame.h / 2 }, list);
  prefs.screenName = screen.name;
  prefs.along = isVertical(edge) ? frame.y : frame.x;
  if (edge === "floating") {
    prefs.floatingX = frame.x;
...
async function applyLayout(payload) {
  ...
  if (payload.edge) prefs.edge = payload.edge;
  if (typeof payload.along === "number") prefs.along = payload.along;
...
async function startBar() {
  ...
  document.addEventListener("pointerup", (e) => {
    if (e.button === 0) invoke("set_pointer", { left: false }).catch(() => {});
  });
